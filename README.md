...
![Тесты](https://img.shields.io/badge/headless--тесты-117%20зелёных-success)
...
уворота с **10 паттернами атак** (рой, ливень, стены костей, вихрь,
преследователи, лучи-телеграфы, шквал ветра, «забор» с дрейфующей щелью,
мульти-залпы и **СИНЯЯ ДУША** — гравитация и прыжки, как у Санса), которые
**комбинируются и переключаются** внутри одной фазы. Бои боссов — с
драматургией: интро → разгон → **фаза ярости** на 50% HP → финал; темп и
скорость растут от хода к ходу. Снаряды: цвет = скорость, 🟢 безвредные
обманки, 🔵 синие (замри!) и 🟠 оранжевые (беги!). Простоев нет: между
волнами капает «морось», в середине атаки случаются спайки-сюрпризы, повторы
паттернов мутируют, а хитбокс сердца **динамический** — в плотном аду
сжимается, чтобы щели оставались честно проходимыми. Грейзы «впритирку»,
стоп-кадр и вспышка при ударе. У каждого биома — своя физика уворота
(лёд скользит, вулкан жжёт стены, болото вязкое).

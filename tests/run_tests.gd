...
func _test_data_integrity() -> void:
    print("\n-- данные --")
    check(DataDB.items.size() == 118, "118 предметов (получено %d)" % DataDB.items.size())
    check(DataDB.locations.size() == 35, "35 локаций (%d)" % DataDB.locations.size())
    check(DataDB.npcs.size() == 15, "15 NPC (%d)" % DataDB.npcs.size())
    var bad := 0
...
func _test_balance() -> void:
    ...
    check(be.evade_chance() == 55, "зелье уворота +35%%")
    be.evade_bonus = 200
    check(be.evade_chance() == 75, "уворот клампится на 75%%")

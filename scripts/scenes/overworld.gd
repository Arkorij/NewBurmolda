...
func _ready() -> void:
    ...
    add_child(layer)
    hud_bg = ColorRect.new()
    hud_bg.color = Color(0, 0, 0, 0.6)
    hud_bg.position = Vector2(0, 0)
    hud_bg.size = Vector2(640, 44)
    layer.add_child(hud_bg)
    hud = Label.new()
...
func _update_camera() -> void:
    ...
    cam.limit_left = 0
    cam.limit_top = 0
    cam.limit_right = int(grid[0].length()) * TILE
    cam.limit_bottom = grid.size() * TILE
